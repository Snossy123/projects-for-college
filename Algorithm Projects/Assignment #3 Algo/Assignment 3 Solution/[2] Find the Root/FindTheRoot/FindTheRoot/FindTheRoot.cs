...
namespace FindTheRoot
{
    class FindTheRoot
    {
        ...
        {
            double a = 0.0, b = 1.0 , fapproximatezero = f(p, q, r, s, t, u, -0.00000000001),
                   fapproximateone = f(p, q, r, s, t, u, 1.000000000001);
            if (fapproximatezero < 0 || fapproximateone > 0) {return -1; }
            while (true)
            {
                //get the middle of the curve and result of the function at middle
                double c = (a + b) / 2 , fc = f(p, q, r, s, t, u, c); ;
                //check if the function is between the boundaries or no
                if (fc > -eps && fc < eps) { return c; }
