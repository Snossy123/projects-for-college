...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	{
		int ret=pf_add_empty_env_page(e,i,0);
		if(ret!=0)
		{
			panic("  error for allocating ... ");
		}
	}
}
void moveToWS (struct Env* e,struct WorkingSetElement * element, struct WS_List * removedList){

	unmap_frame(e->env_page_directory,(void *)element->virtual_address);
	LIST_REMOVE(removedList,element);
	pt_set_page_permissions(e,element->virtual_address,0,PERM_PRESENT);
	element->empty = 1;
	LIST_INSERT_HEAD(&(e->PageWorkingSetList), element);

}
void freePageTable (struct Env* e,struct WorkingSetElement * elem){
	bool flagPage = 0;
	uint32 *ptr_page_table;
//	freeing from page table .. entry by entry.

	struct Frame_Info *frame = get_frame_info(e->env_page_directory,(void*)elem->virtual_address,&ptr_page_table);

	for(int k=0; k<1024; k++)
	{
		if(ptr_page_table[k]!=0){
			flagPage = 1;
			break;
		}
	}
//	if exist elements is n used decrement refrences and free frame
	if(!flagPage){
//		we need to free page table .. we first get the physical address to pass it to func : free frame and remove entries in p dir
		uint32 physical_address =(uint32)ptr_page_table - KERNEL_BASE;
		struct Frame_Info * t=to_frame_info(physical_address);
		//this func check if refrences == 0 and free fram directly after check.
		decrement_references(t);
		pd_clear_page_dir_entry(e, elem->virtual_address);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//2. Free ONLY pages that are resident in the working set from the memory
		// search and remove(free) from active list
		struct WorkingSetElement * element;
		LIST_FOREACH(element, &(e->ActiveList))
			 {

				if(element->virtual_address >= virtual_address && element->virtual_address <(virtual_address+size)){

						moveToWS(e, element, &e->ActiveList);
						struct WorkingSetElement * firSec=LIST_FIRST(&e->SecondList);
						if(LIST_SIZE(&e->SecondList)>0)
						{
							LIST_REMOVE(&(e->SecondList),firSec);
							pt_set_page_permissions(e,firSec->virtual_address,PERM_PRESENT|PERM_WRITEABLE|PERM_USER,0);
							LIST_INSERT_TAIL(&e->ActiveList,firSec);
						}
							freePageTable(e, element);
				}

			}

		// search and remove(free) from second list
		struct WorkingSetElement * element2;
		LIST_FOREACH(element2, &(e->SecondList))
			 {

				if(element2->virtual_address >= virtual_address && element2->virtual_address < (virtual_address+size))
				{
						moveToWS(e, element2, &e->SecondList);
						freePageTable(e, element2);
 				}
